...
class Listener:
    ...
    def __init__(self, client):
        ...
        self._logger = logging.getLogger("queue_processor")
    ...
    def data_ready(self):
        ...
        self._logger.info("Data ready for processing run %s on %s", run_no,
                          instrument_name)
        ...
        if instrument.is_paused:
            status = self._utils.status.get_skipped()
        else:
            status = self._utils.status.get_queued()

        # If there has already been an autoreduction job for this run,
        # we need to know it so we can
        # increase the version by 1 for this job. However, if not then we
        # will set it to -1 which
        ...
        self.message.run_version = reduction_run.run_version  # pylint:
        # disable=no-member
        ...
        if not variables:
            self._logger.warning(
                ...
                instrument.name, self.message.run_number)
