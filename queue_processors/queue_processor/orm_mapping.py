...
from queue_processors.queue_processor.base import Base, metadata, engine
...
class Software(Base):
    __table__ = Table('reduction_viewer_software', metadata, autoload=True, autoload_with=engine)
...
class ReductionRun(Base):
    ...
    software = relationship('Software', foreign_keys='ReductionRun.software_id')
    data_location = relationship('DataLocation',
                                 primaryjoin='(DataLocation.reduction_run_id == ReductionRun.id)')
...
# A special class that joins the InstrumentVariable and Variable tables. This means that one can
# access all parts of both tables whilst only needing to produce one object.
class InstrumentJoin(Base):
    instrument_variable_table = Table('reduction_variables_instrumentvariable', metadata,
                                      Column('variable_ptr_id', Integer,
                                             ForeignKey('reduction_variables_variable.id'),
                                             primary_key=True),
                                      Column('instrument_id', Integer,
                                             ForeignKey('reduction_viewer_instrument.id')),
...
# A special class that joins the RunVariable and Variable tables. This means that one can access
# all parts of both tables whilst only needing to produce one object.
class RunJoin(Base):
...
class InstrumentVariable(Base):
    __table__ = Table('reduction_variables_instrumentvariable',
                      metadata,
                      autoload=True,
                      autoload_with=engine)
...
class Notification(Base):
    __table__ = Table('reduction_viewer_notification',
                      metadata,
                      autoload=True,
                      autoload_with=engine)
