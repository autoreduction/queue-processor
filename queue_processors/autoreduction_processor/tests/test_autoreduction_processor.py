...
class TestAutoreductionProcessorListener(unittest.TestCase):
    ...
    @patch(DIR + '.autoreduction_logging_setup.logger.error')
    ...
    def test_add_process(self):
        self.listener.add_process('fake-process', self.message)
        self.assertEqual(self.listener.proc_list, ['fake-process'])
        self.assertEqual(self.listener.rb_list, [self.message.rb_number])

    def test_should_proceed_false(self):
        self.listener.rb_list = [self.message.rb_number]
        self.assertFalse(self.listener.should_proceed(self.message))

    def test_should_proceed_true(self):
        self.assertTrue(self.listener.should_proceed(self.message))

    def test_run_tuple_no_version(self):
        self.assertEqual(('111', 0), self.listener.run_tuple(self.message))

    def test_run_tuple_with_version(self):
        self.message.run_version = '1'
        self.assertEqual(('111', '1'), self.listener.run_tuple(self.message))

    def test_add_cancel(self):
        self.listener.add_cancel(self.message)
        self.assertEqual(self.listener.cancel_list, [(self.message.run_number, 0)])

    def test_add_cancel_when_exists(self):
        """ Ensure that runs are not duplicated in the cancel list """
        self.listener.cancel_list = [(self.message.run_number, 0)]
        self.listener.add_cancel(self.message)
        self.assertEqual(self.listener.cancel_list, [(self.message.run_number, 0)])

    def test_should_cancel_false(self):
        self.assertFalse(self.listener.should_cancel(self.message))

    def test_should_cancel_true(self):
        self.listener.cancel_list = [(self.message.run_number, 0)]
        self.assertTrue(self.listener.should_cancel(self.message))

    def test_cancel_run(self):
        self.listener.cancel_list = [(self.message.run_number, 0)]
        self.listener.cancel_run(self.message)
