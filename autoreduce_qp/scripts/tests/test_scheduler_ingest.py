...
class TestCycle(unittest.TestCase):
    ...
    def test_init(self):
        ...
        """
        cycle = Cycle(self.test_cycle_values["name"], self.test_cycle_values["start"], self.test_cycle_values["end"])
    ...
    def test_add_maintenance_day(self):
        ...
        """
        cycle = Cycle(self.test_cycle_values["name"], self.test_cycle_values["start"], self.test_cycle_values["end"])
        cycle.add_maintenance_day(self.test_maintenance_day_values["start"], self.test_maintenance_day_values["end"])
        self.assertEqual(1, len(cycle.maintenance_days))
        self.assertIsInstance(cycle.maintenance_days[0], MaintenanceDay)
        self.assertEqual(cycle.maintenance_days[0].start, self.test_maintenance_day_values["start"])
        self.assertEqual(cycle.maintenance_days[0].end, self.test_maintenance_day_values["end"])
...
class TestSchedulerDataProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        number_of_cycles = 3
        self.test_cycle_data = self.create_cycle_data(length=number_of_cycles, initial_start=cycles_start)
        self.test_maintenance_dict = {
            "before_cycles":
            self.create_maintenance_day_from_start_date(cycles_start + relativedelta(days=-1)),
            "within_first_cycle":
            self.create_maintenance_day_from_start_date(cycles_start + relativedelta(weeks=2)),
            "after_cycles":
            self.create_maintenance_day_from_start_date(cycles_start + relativedelta(months=number_of_cycles, days=1))
        }
    ...
    def create_cycle_data(length, initial_start):
        ...
        for index in range(length):
            ...
            id_ = str(index + 1)
            cycles.append({"id": id_, "name": f"000{id_}/x", "start": start_date, "end": end_date})
    ...
    def create_cycle_from_dict(cycle_dict):
        ...
        return Cycle(cycle_dict["name"], cycle_dict["start"], cycle_dict["end"])
