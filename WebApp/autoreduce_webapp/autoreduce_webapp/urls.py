...
urlpatterns = [
    ...
    url(r'^runs/list/(?P<instrument_name>\w+)/$',
        reduction_viewer_views.load_runs, name='load_runs'),
    url(r'^runs/(?P<run_number>[0-9]+)(?:/(?P<run_version>[0-9]+))?/$',
        reduction_viewer_views.run_summary, name='run_summary'),
    url(r'^runs/(?P<instrument>\w+)/confirmation/$',
        reduction_variables_views.run_confirmation, name='run_confirmation'),

    # ===========================INSTRUMENT========================== #
    url(r'^instrument/(?P<instrument>\w+)/$',
        reduction_viewer_views.instrument_summary, name='instrument_summary'),
    url(r'^instrument/(?P<instrument>\w+)/pause$',
        reduction_viewer_views.instrument_pause, name='instrument_pause'),
    url(r'^instrument/(?P<instrument>\w+)/submit_runs/$',
        reduction_variables_views.submit_runs, name='instrument_submit_runs'),
    ...
    url(r'^instrument/(?P<instrument>\w+)/variables/experiment/(?P<experiment_reference>[0-9]+)/$',
        reduction_variables_views.instrument_variables, name='instrument_variables_by_experiment'),
